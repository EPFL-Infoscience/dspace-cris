...
public class RequestItemServiceImpl implements RequestItemService {
    ...
    public void setAccessExpiry(RequestItem requestItem, String dateOrDelta) {
        try {
            setAccessExpiry(requestItem, parseDateOrDelta(dateOrDelta, requestItem.getDecision_date().toInstant()));
    ...
    public static Instant parseDateOrDelta(String dateOrDelta, Instant decisionDate)
        ...
        Date parsedExpiryDateAsDate = MultiFormatDateParser.parse(dateOrDelta);
        ZonedDateTime parsedExpiryDate = parsedExpiryDateAsDate != null ?
                parsedExpiryDateAsDate.toInstant().atZone(ZoneId.of("UTC")) : null;
        ...
        if (parsedExpiryDate == null) {
            ...
            dateMathParser.setNow(Date.from(decisionDate.atZone(ZoneOffset.UTC).toInstant()));

            // Parse the delta (e.g. +7DAYS) and set the new access expiry date
            return dateMathParser.parseMath(dateOrDelta).toInstant();
        } else {
